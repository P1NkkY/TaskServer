...
OfstreamSink::OfstreamSink(FileSinkConfig config) : config_(std::move(config)) {
    if (config_.filename_.empty()) {
...
void OfstreamSink::CreateSink() {
    ...
    boost::shared_ptr<std::ofstream> file_stream =
        boost::make_shared<std::ofstream>(config_.filename_,
                                          std::ofstream::app);

    if (!file_stream->is_open()) {
        throw std::runtime_error("Can not open log file: " + config_.filename_);
    }
